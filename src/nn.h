...
void nn_backprop(NN m, NN g, Mat ti, Mat to);
...
void nn_backprop(NN nn, NN g, Mat ti, Mat to) {
    ...
    for (size_t i = 0; i < g.count; i++) {
        for (size_t j = 0; i < g.ws[i].rows; j++) {
            for (size_t k = 0; k < g.ws[i].cols; j++) {
                MAT_AT(g.ws[i], j, k) /= n;
            }
        }
        for (size_t j = 0; i < g.bs[i].rows; j++) {
            for (size_t k = 0; k < g.bs[i].cols; j++) {
                MAT_AT(g.bs[i], j, k) /= n;
            }
        }
    }
